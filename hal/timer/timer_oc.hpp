...
class TimerOC final : public pwmIntf, public TimerOut {
   ...
   public:
    TimerOC(TIM_TypeDef *_instance, const ChannelIndex _channel)
        : TimerOut(_instance, _channel), cvr_(from_channel_to_cvr(_instance, _channel)), arr_(_instance->ATRLR) {}
...
class TimerOCN final : public TimerOut {
   public:
    TimerOCN(TIM_TypeDef *instance, const ChannelIndex ch) : TimerOut(instance, ch) {}
...
class TimerOcMirror final : public pwmIntf {
   ...
   public:
    TimerOcMirror(TimerOC &oc, TimerOCN &ocn) : oc_(oc), ocn_(ocn) {}
...
class TimerOcPair final : public pwmIntf {
   ...
   public:
    TimerOcPair(TimerOC &oc, TimerOC &ocn) : oc_(oc), ocn_(ocn) {}
    void set_duty(const real_t value) {
        const bool is_minus = signbit(value);
        const auto abs_value = inverse_ ? (1 - abs(value)) : abs(value);
        const auto zero_value = real_t(inverse_);
    ...
    void inverse(const Enable en = EN) { inverse_ = en.to_bool(); }
};
}  // namespace ymd::hal
