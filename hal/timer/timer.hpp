...
#define DEF_GENERIC_TIMER_FRIEND_DECL(x) friend void ::TIM##x##_IRQHandler(void);
#define DEF_BASIC_TIMER_FRIEND_DECL(x) friend void ::TIM##x##_IRQHandler(void);
...
class BasicTimer {
   ...
   public:
    BasicTimer(TIM_TypeDef* instance) : instance_(instance) {}
    void init(const uint32_t ferg, const Mode mode = Mode::Up, const Enable en = EN);
    ...
    void attach(const IT it, const NvicPriority& priority, Fn&& cb, const Enable en = EN) {
        bind_cb(it, std::forward<Fn>(cb));
        enable_it(it, priority, en);
    }

    void detach(const IT it, const NvicPriority& priority) { attach(it, priority, nullptr, DISEN); }

    template <typename Fn>
    void bind_cb(const IT it, Fn&& cb) {
        get_callback(it) = std::forward<Fn>(cb);
    }
...
class GenericTimer : public BasicTimer {
   ...
   public:
    GenericTimer(TIM_TypeDef* instance)
        : BasicTimer(instance),
          channels(TimerOC(instance_, TimerChannel::ChannelIndex::CH1), TimerOC(instance_, TimerChannel::ChannelIndex::CH2),
                   TimerOC(instance_, TimerChannel::ChannelIndex::CH3), TimerOC(instance_, TimerChannel::ChannelIndex::CH4)) {}

    void init_as_encoder(const Mode mode = Mode::Up);
    void enable_single(const Enable en = EN);
    ...
    volatile uint16_t& cvr() {
        switch (I) {
            case 1:
                return instance_->CH1CVR;
            case 2:
                return instance_->CH2CVR;
            case 3:
                return instance_->CH3CVR;
            case 4:
                return instance_->CH4CVR;
            default:
...
class AdvancedTimer : public GenericTimer {
   ...
   public:
    ...
    AdvancedTimer(TIM_TypeDef* instance)
        : GenericTimer(instance),
          n_channels{TimerOCN(instance_, TimerChannel::ChannelIndex::CH1N), TimerOCN(instance_, TimerChannel::ChannelIndex::CH2N),
    ...
    template <size_t I>
        requires(I >= 1 and I <= 3)
    TimerOCN& ocn() {
        return n_channels[I - 1];
    }
