...
struct __static_string {
    static constexpr char str[sizeof...(Args)] = {Args...};

    constexpr explicit operator const char*() const { return __static_string::str; }

    constexpr explicit operator std::string_view() const { return std::string_view(__static_string::str, sizeof...(Args)); }
};
...
constexpr std::string_view _enum_item_name() {
    ...
    constexpr std::size_t end = [&]() {
        for (std::size_t i = full_name.size() - 1; i > 1; i--) {
            const char chr = full_name[i];
            if (chr == ';') {
                return i;
            }
...
constexpr bool _enum_is_valid() {
    ...
    constexpr std::size_t begin = [&]() {
        for (std::size_t i = 0; i < full_name.size(); i++) {
            const char chr = full_name[i];
            if (chr == ' ' || chr == ')') return i + 1;
        }
    }();

    constexpr std::size_t end = [&]() {
        for (std::size_t i = 0; i < full_name.size(); i++)
            if (full_name[i] == ']') return i;
    }();

    constexpr auto name = full_name.substr(begin, end - begin);
...
struct _tuple_element_offset_v {
    static constexpr size_t value = _tuple_element_offset_v<N - 1, Tup>::value + sizeof(std::tuple_element_t<N - 1, Tup>);
};
