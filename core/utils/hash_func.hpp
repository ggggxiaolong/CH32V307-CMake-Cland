...
struct Hasher {
   public:
    __inline scexpr uint32_t hash_uiml32(char const* data, size_t length) {
        ...
        for (size_t i = 0; i < alignedLen; ++i) h = (h << 5) - h + (reinterpret_cast<const uint32_t*>(data)[i]);
        for (size_t i = alignedLen << 2; i < strLength; ++i) h = (h << 5) - h + uint8_t(data[i]);
        return h;
    }
...
__inline constexpr uint32_t operator"" _ha(char const* p, const size_t size) { return hash_impl(p, size); }