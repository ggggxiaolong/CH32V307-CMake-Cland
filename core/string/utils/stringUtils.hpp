...
#include "core/math/iq/universal/_IQN.hpp"
...
int kmp_find(char *str, const size_t src_len, const char *match,
             const size_t match_len);
void str_replace(char *src, const size_t src_len, const char *match,
                 const char *replace, const size_t dst_len);
...
size_t iutoa(uint32_t value, char *str, uint8_t radix);
...
__inline float atod(const char *str) { return atof(str, strlen(str)); }

static __fast_inline constexpr void itoas(uint32_t value, char *str,
                                          uint8_t radix, int8_t i) {
  i -= 1;
  do {
...
template <size_t Q> iq_t<Q> atoq(const char *str, const size_t len) {
