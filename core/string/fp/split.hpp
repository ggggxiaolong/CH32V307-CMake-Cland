...
class SplitView : public std::ranges::view_interface<SplitView> {
   ...
   public:
    ...
    class iterator {
       ...
       public:
        ...
        StringView operator*() const {
            ...
            return current_.substr(0, pos.value());
        }
        ...
        iterator &operator++() {
            ...
            if (pos == size_t(-1)) {
                ...
                current_ = current_.substr(pos.value() + 1);
            }
            if (max_pieces_) {
                ++count_;
            }
    ...
    iterator begin() { return iterator(base_, delimiter_, max_pieces_); }
...
class SplitIterator {
   ...
   public:
    ...
    constexpr SplitIterator(const StringView &str, char delimiter, size_t max_pieces = 0)
        : data_(str.data()), size_(str.size()), delimiter_(delimiter), max_pieces_(max_pieces), remaining_(str.size()) {}
...
static constexpr SplitIterator split(const StringView str, char delimiter, size_t max_pieces = 0) {
