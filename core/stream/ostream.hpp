...
template <typename T>
concept has_derive_debug_dispatcher = requires(OutputStream& os, const T& value) { derive_debug_dispatcher<T>::call(os, value); };
...
class OutputStream : public OutputStreamIntf {
   ...
   private:
    ...
    __fast_inline void print_splt() { write(config_.splitter, sp_len); }
    ...
    struct Buf {
        ...
        // 清空缓冲区（不发送数据）
        __fast_inline void clear() { size = 0; }
    };
   ...
   public:
    ...
    void write(const char* pbuf, const size_t len) {
        buf_.push(std::span<const char>(pbuf, len), [this](const std::span<const char> pbuf) { this->sendout(pbuf); });
    }
    ...
    uint8_t indent() const { return config_.indent; }
    ...
    __inline OutputStream& operator<<(char* str) {
        if (str) checked_write(str, strlen(str));
        return *this;
    }
    ...
    OutputStream& operator<<(const std::_Setprecision n) {
        config_.eps = n._M_n;
        return *this;
    }
    OutputStream& operator<<(const std::_Setbase n) {
        config_.radix = n._M_base;
        return *this;
    }
    OutputStream& operator<<(const Endl) {
        this->print_endl();
        return *this;
    }
    ...
    OutputStream& operator<<(const Splitter) {
        print_splt();
        return *this;
    }
    ...
    OutputStream& operator<<(const Brackets<chr>) {
        if (!config_.no_brackets) {
            write(chr);
        }
        return *this;
    }
    OutputStream& operator<<(const std::source_location& loc) {
        print_source_loc(loc);
        return *this;
    }
   ...
   private:
    ...
    void print_tuple(const std::tuple<Args...>& t) {
        ...
        *this << brackets<'('>();
        std::apply(
            [&](const auto&... args) { ((tupleSize > 1 && &args != &std::get<tupleSize - 1>(t) ? (*this << args << ',') : (*this << args)), ...); },
            t);
        *this << brackets<')'>();
    }
   ...
   public:
    ...
    class __Guard {
       ...
       public:
        ...
        ~__Guard() { os_.reconf(config_); }
    };

    [[nodiscard]] __Guard create_guard() { return __Guard(*this); }
};
...
class OutputStreamByRoute final : public OutputStream {
   ...
   public:
    ...
    OutputStreamByRoute(Route&& route) : p_route_(std::move(route)) { ; }

    size_t pending() const {
        if (unlikely(!p_route_))
            while (true);
        return p_route_->pending();
    }

    void retarget(Route&& p_route) { p_route_ = std::move(p_route); }
