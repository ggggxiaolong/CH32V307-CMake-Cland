...
template <typename... Args>
__fast_inline void DEBUG_PRINTLN_IDEL(Args&&... args) {
    if (ymd::DEBUGGER.pending() == 0) {
        ymd::DEBUGGER.println(std::forward<Args>(args)...);
    }
}
...
template <typename... Args>
__attribute__((noreturn)) __fast_inline void PANIC_NSRC(Args&&... args) {
    sys::trip();
...
template <typename... Args>
__attribute__((noreturn)) PANIC<Args...> __PANIC_EXPLICIT_SOURCE(const std::source_location& loc, Args&&... args) {
...
struct ASSERT {
   ...
   public:
    constexpr ASSERT(Texpr&& expr, Args&&... args, const std::source_location& loc = std::source_location::current()) : is_ok_(bool(expr)) {
        if (!is_ok_) {
            PANIC<Args...>(std::forward<Args>(args)..., loc);
        }
    }
...
extern "C" {
__attribute__((used)) int _write(int file, char* pbuf, int len);

__attribute__((used)) void* _sbrk(ptrdiff_t incr);

__attribute__((used)) __attribute__((noreturn)) void _exit(int status);
