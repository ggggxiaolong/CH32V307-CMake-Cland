...
#include "sdk/Peripheral/inc/ch32v30x_conf.h"
...
extern void DEBUG_PRINTLN(Args &&...args);
...
uint64_t sys::chip::get_chip_id() {
    static uint32_t chip_id[2]{*(volatile uint32_t *)0x1FFFF7E8, *(volatile uint32_t *)0x1FFFF7EC};
...
uint32_t sys::clock::get_system_freq() {
    M_CLOCK_TYPEDEF clocks;
    M_RCC_CLK_GETTER(&clocks);
    return M_RCC_SYSCLK(clocks);
}

uint32_t sys::clock::get_apb1_freq() {
    M_CLOCK_TYPEDEF clocks;
    M_RCC_CLK_GETTER(&clocks);
    return M_RCC_PCLK1(clocks);
}
...
uint32_t sys::chip::get_dev_id() { return DBGMCU_GetDEVID(); }

uint32_t sys::chip::get_rev_id() { return DBGMCU_GetREVID(); }
...
void sys::clock::set_ahb_freq(const uint32_t freq) { set_ahb_div(get_ahb_freq() / freq); }

void sys::clock::set_apb1_freq(const uint32_t freq) { set_apb1_div(get_apb1_freq() / freq); }

void sys::clock::set_apb2_freq(const uint32_t freq) { set_apb2_div(get_apb2_freq() / freq); }
...
void sys::exception::disable_interrupt() { __disable_irq(); }

void sys::exception::enable_interrupt() { __enable_irq(); }

// 定义宏：读取寄存器的值
#define READ_REGISTER(reg)                                 \
    ({                                                     \
        uint32_t ret;                                      \
        __asm__ volatile("mv %0, " #reg "\n" : "=r"(ret)); \
        ret;                                               \
    })
...
void sys::dump() {
    ...
    // 打印寄存器值
    DEBUG_PRINTLN("system exited, reg dump:", "mstatus: ", cpu_mstatus, "x1: ", cpu_x1, "x3: ", cpu_x3);
}
