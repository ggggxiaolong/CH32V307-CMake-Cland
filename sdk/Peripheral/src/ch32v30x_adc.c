...
void ADC_Init(ADC_TypeDef *ADCx, ADC_InitTypeDef *ADC_InitStruct) {
    ...
    tmpreg1 |= (uint32_t)(ADC_InitStruct->ADC_Mode | (uint32_t)ADC_InitStruct->ADC_OutputBuffer | (uint32_t)ADC_InitStruct->ADC_Pga |
 ...
 * @return  none
 */
void ADC_ResetCalibration(ADC_TypeDef *ADCx) { ADCx->CTLR2 |= CTLR2_RSTCAL_Set; }
 ...
 */
uint16_t ADC_GetConversionValue(ADC_TypeDef *ADCx) { return (uint16_t)ADCx->RDATAR; }
 ...
 */
uint32_t ADC_GetDualModeConversionValue(void) { return (*(__IO uint32_t *)RDATAR_ADDRESS); }
 ...
 * @return  none
 */
void ADC_ClearFlag(ADC_TypeDef *ADCx, uint8_t ADC_FLAG) { ADCx->STATR = ~(uint32_t)ADC_FLAG; }
